...
class Moderation(commands.Cog):
    ...
    async def ban(self, interaction: discord.Interaction, member: discord.Member, reason: str = None, notifymember: bool = True):
        if interaction.user.top_role.position < member.top_role.position:
            await interaction.response.send_message(f"{interaction.user.mention}, You cannot ban a member that is higher than you.", ephemeral=True)
            return
        ...
        await interaction.response.send_message(f"{interaction.user.mention}, You have banned {member.mention}.", ephemeral=True)
    ...
    async def kick(self, interaction: discord.Interaction, member: discord.Member, reason: str = None, notifymember: bool = True):
        if interaction.user.top_role.position < member.top_role.position:
            await interaction.response.send_message(f"{interaction.user.mention}, You cannot kick a member that is higher than you.", ephemeral=True)
            return
        ...
        await interaction.response.send_message(f"{interaction.user.mention}, You have kicked {member.mention}.", ephemeral=True)
